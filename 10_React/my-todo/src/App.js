...
function App() {
  ...
  // 로컬 스토리지에 저장(주의: DB가 아님, DB처럼 쓰면 안 됨!!)
  ...
  // Quiz: state가 아닌 이유? id 값은 렌더링되는 정보가 아니기 때문 (화면에 보이지도 않고, 이 값이 바뀐다고 해서 컴포넌트가 재렌더링 될 필요도 없음)
  ...
  const handleInsert = useCallback((text) => {
    ...
    // 방법 1 - 이전까지 쓰던 방법 // 이 방식으로 썼을 경우 todos를 의존성 배열에 넣어주어야 함
    ...
    setTodos(todos => todos.concat(todo)); // 새로운 배열을 반환함 // 매개변수로 이전 state를 가져옴

    nextId.current += 1; // nextId에 1씩 더하기
  }, []);

  const handleRemove = useCallback((id) => {
    // 방법 1 // 이 방식으로 썼을 경우 todos를 의존성 배열에 넣어주어야 함
    // const copyTodos = [...todos];
